...
lessons_with_difficulty AS (
  SELECT 
    ...
    cd.status,
    cd.IS_CANCELED,
    cd.LAST_CANCEL_DATETIME,
    cd.STATUS_12M,
    cd.IS_CANCELED_12M,
    cd.CANCEL_MONTH,
    cd.TOTAL_SUBSCRIPTION_DAYS
...
   min(FIRST_SUBSCRIPTION_PLUS_12M_KST) AS FIRST_SUBSCRIPTION_PLUS_12M_KST,
   ...
   max(LAST_CANCEL_DATETIME) AS LAST_CANCEL_DATETIME,
   max(CANCEL_MONTH) AS CANCEL_MONTH,
   max(TOTAL_SUBSCRIPTION_DAYS) AS TOTAL_SUBSCRIPTION_DAYS
