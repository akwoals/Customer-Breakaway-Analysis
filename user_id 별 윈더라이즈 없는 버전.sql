...
content_difficulty AS (
 SELECT DISTINCT
  ...
  left join `team_pjt.subscription_status` as rcy on sc.user_id = rcy.user_id
  where rcy.user_id is not null
),
...
stats AS (
  SELECT
    ...
    AVG(total_completed_contents) AS mean_completions,
    STDDEV_POP(total_completed_contents) AS std_completions
  FROM user_profiles_base
),
...
user_profiles_final AS (
  SELECT 
    ...
    (upw.total_completed_contents - s.mean_completions) / NULLIF(s.std_completions, 0) AS z_score_completions,
    
    
    (upw.weighted_mean_difficulty - s.mean_difficulty) / NULLIF(s.std_difficulty, 0) +
    (upw.total_completed_contents - s.mean_completions) / NULLIF(s.std_completions, 0) AS total_skill_score
    
  FROM user_profiles_base AS upw
...
SELECT 
  ...
  CASE
    ...
    WHEN z_score_completions <= 1.5 AND z_score_difficulty > 1.5 THEN '여유'
    ELSE '느린 출발형'
  END AS user_persona,
  ...
  STATUS,
  IS_CANCELED,
  LAST_CANCEL_DATETIME,
  CANCEL_MONTH,
  TOTAL_SUBSCRIPTION_DAYS
FROM user_profiles_final as upj
inner join `team_pjt.subscribe_total_ms` as ms on upj.user_id = ms.USER_ID
